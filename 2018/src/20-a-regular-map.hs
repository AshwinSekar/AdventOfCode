...
data PathState a = PathState
  { visited :: Map (Point a) a,
    paths :: Map (Point a) a
  }
  deriving (Show)
...
visit = lift $ modify (\s -> s {visited = visit' s})
  where
    visit' (PathState v p) = Map.unionWith min v p

move :: Integral a => Point a -> Parser a ()
move (x, y) = lift $ modify (\s -> s {paths = move' s})
  where
    move' PathState {paths} = (+ 1) <$> Map.mapKeysWith min pPlus paths
    pPlus (x', y') = (x + x', y + y')
...
splitParser = do
  ...
  where
    combine (PathState v p) (PathState v' p') = PathState (Map.unionWith min v v') (Map.unionWith min p p')
...
main = do
  ...
  where
    initial = PathState Map.empty $ Map.singleton (0, 0) 0
