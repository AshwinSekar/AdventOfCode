...
instrParser = do
  op <- choice [ADDR <$ string "addr" , ADDI <$ string "addi"
              ...
              , GTIR <$ string "gtir" , GTRI <$ string "gtri"
              , GTRR <$ string "gtrr" , EQIR <$ string "eqir"
              , EQRI <$ string "eqri" , EQRR <$ string "eqrr"]
  [a, b, c] <- count 3 (space *> decimal)
  return $ Instr op a b c

parser :: Parser (Int, Map Int Instr)
parser = (,) <$> between (string "#ip ") newline decimal
