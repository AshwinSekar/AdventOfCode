...
parser = (,) <$> sampleParser `sepEndBy1` newline <* many newline
...
valid samples n = map behave samples
                   & filter ((>= n) . length)
                   & length
...
findCodings samples = go Map.empty partial
  where behaves = map (\ s -> (op $ instr s, Set.fromList $ behave s)) samples
...
go ops ints
  ...
  where (dones, ints') = Map.partition ((== 1) . Set.size) ints
