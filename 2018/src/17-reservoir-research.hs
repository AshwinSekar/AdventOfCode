...
data Clay a = X a (a, a) | Y a (a, a)

data Status = Clay | LRest | RRest | Rest | Flow | Sand deriving (Eq)
...
clayParser = do
  ...
  xy <$> between (string "=") (string ", ") decimal
     <*> ((,) <$> between (anySingle *> string "=") (string "..") decimal
              <*> decimal)

parser :: Integral a => Parser (Clays a)
parser = many clayParser
          <&> partition isX
          <&> both (map unwrap)
  where isX (X _ _) = True
        isX (Y _ _) = False
        unwrap (X k v) = (k, v)
        unwrap (Y k v) = (k, v)
...
main = do
  ...
  where findMinY (xClay, yClay) = min (minimum $ map (fst . snd) xClay) (minimum $ map fst yClay)
