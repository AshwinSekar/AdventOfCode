{-# LANGUAGE ViewPatterns, TupleSections, TypeApplications #-}
import Utils
...
parser = liftA2 (:|) pointParser (pointParser `sepEndBy` space)
  where pointParser = Point <$> liftA2 (,) (decimal <* string ", ") (decimal <* newline)
...
main = do
  ...
  let bound = unwrap $ sconcat points
      ...
      p1 = runSTUArray (getFiniteAreas @Int pmap bound)
            & elems
            & maximum
...
findClosest pmap p = case sortBy dist dists of
                       (p0, d0):(p1, d1):_
                         | d0 == d1  -> Nothing
                         | otherwise -> Just p0
  where dist (_, d) (_, d') = compare d d'
