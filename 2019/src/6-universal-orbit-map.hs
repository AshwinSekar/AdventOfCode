...
main = do
  ...
  where split s = let [a, b] = splitString (== ')') s in (b, a)


totalOrbit :: Map.Map String String -> Int
totalOrbit adjList = sum orbits
  where orbits = execState (mapM_ (orbit adjList) (Map.keys adjList)) Map.empty
...
totalTransfers adjList =
  ...
  where meq a b = if a == b then Nothing else Just(a, b)
