...
findStable step seen seats
  ...
  where seats' = Map.mapWithKey (step seats) seats
        seen' = Set.insert seats seen

seatFlip adj occN seats pos 'L' | count (=='#') (adj seats pos) == 0 = '#'
seatFlip adj occN seats pos '#' | count (=='#') (adj seats pos) >= occN = 'L'
...
ray8Adj seats pos = mapMaybe (look pos) eightDirs
  where look pos dir = case Map.lookup (psum pos dir) seats of
                       Nothing    -> Nothing
