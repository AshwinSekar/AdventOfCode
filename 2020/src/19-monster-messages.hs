...
main = do
  ...
  let rulesP  = Map.map (mkParser rulesP) rules
      rule0   = whenM (liftA2 (<=) (length <$> some (try (rulesP ! 42))) (length <$> some (rulesP ! 31))) (fail "")
