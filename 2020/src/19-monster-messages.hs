...
ruleParser =
  liftA2
    ...
    (choice
       ...
       , Right <$>
         some (try $ decimal <* notFollowedBy (symbol ":")) `sepBy` symbol "|"
       ])

parser =
  liftA2
    (,)
    (Map.fromList <$> some ruleParser)
    (some letterChar `sepEndBy1` newline)

mkParser ::
     Map.Map Int (Parsec Void String ())
  -> Either Char [[Int]]
  -> Parsec Void String ()
  ...
  let rulesP = Map.map (mkParser rulesP) rules
      rule0 =
        whenM
          (liftA2
             (<=)
             (length <$> some (try (rulesP ! 42)))
             (length <$> some (rulesP ! 31)))
          (fail "")
