...
nav (theta, pos) ("F", s)      = (theta, theta & sin . d2r &&& cos . d2r & both (* s) & psum pos)
...
main = do
  (p1, p2) <-
    getFile "data/12-puzzle-input" <&> map (second read . splitAt 1) <&> foldl nav (0, (0, 0)) &&&
    foldl navWP ((1, 10), (0, 0)) <&> snd *** snd <&> both (round . uncurry (+) . both abs)
