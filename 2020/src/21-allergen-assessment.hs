...
main = do
  ...
  let poss = Map.fromListWith Set.intersection [(a, Set.fromList ings) | (ings, allergs) <- input, a <- allergs]
  putStrLn $ "Part 1: " ++ show (length [i | (ings, allergs) <- input, i <- ings, i `notElem` Set.unions poss])
