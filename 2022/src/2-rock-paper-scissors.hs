...
handParser =
  choice [symbol "A" $> Rock, symbol "B" $> Paper, symbol "C" $> Scissors]

outcomeParser :: Parser Outcome
outcomeParser =
  choice [symbol "X" $> Loss, symbol "Y" $> Draw, symbol "Z" $> Win]

main :: IO ()
main = do
  handsAndOutcomes <-
    parseFile "data/2-puzzle-input" $ some (liftA2 (,) handParser outcomeParser)
  let hands =
        map
          (\(theirs, outcome) -> (theirs, toEnum $ fromEnum outcome))
          handsAndOutcomes
      p1 =
        sum $
        map (\(theirs, yours) -> fromEnum yours + 1 + score theirs yours) hands
      p2 =
        sum $
        map
          (\(hand, result) ->
             fromEnum (decode hand result) + 1 + 3 * fromEnum result)
          handsAndOutcomes
