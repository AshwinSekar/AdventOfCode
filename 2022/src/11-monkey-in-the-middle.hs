...
import           Utils                (Parser, count, decimal, getFile, lexeme, parseFile, slices,
                                       symbol, word)
...
monkeyParser div3 = do
  ...
  items <- symbol "Starting items:" *> some (try (decimal <* symbol ",") <|> decimal)
  operation <-
    symbol "Operation: new = old" *>
    choice
      [try $ symbol "*" $> (*) <*> decimal, symbol "* old" $> (^ 2), symbol "+" $> (+) <*> decimal]
...
throw :: Int -> (Map.Map Int Int, MonkeyMap) -> Int -> (Map.Map Int Int, MonkeyMap)
...
main :: IO ()
main = do
  monkeys1 <- Map.fromList <$> parseFile "data/11-puzzle-input" (some (monkeyParser True))
  monkeys2 <- Map.fromList <$> parseFile "data/11-puzzle-input" (some (monkeyParser False))
  let n = length monkeys1
      count = Map.fromList ((, 0) <$> [0 .. n - 1])
      (reverse . sort . Map.elems -> mb1, _) = foldl (throw n) (count, monkeys1) [0 .. (20 * n - 1)]
