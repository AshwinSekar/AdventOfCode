...
main :: IO ()
main = do
  tubes <- Map.filter (< 9) . Map.map digitToInt . gridMap <$> getFile "data/9-puzzle-input"
  let isLow p c = c < minimum (neighs p)
      ...
      neigh p = mapMaybe (\d -> (, psum p d) <$> tubes !? psum p d) fourDirs & minimum & snd
      findGrad b p c = fromMaybe (b ! neigh p) (llows !? p)
      basins = Map.mapWithKey (findGrad basins) tubes
      x:y:z:_ = Map.elems llows & map (\c -> mcount (== c) basins) & reverse . sort
